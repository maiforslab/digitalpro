	...
	<div class="modal-card">
		...
		<header class="modal-card-head">
			<div class="is-flex-grow-1">
				<h3 class="title is-3">{{ $t('Share Falah OS') }}</h3>
			</div>
			<div>
			...
			<div class="node-card  mt-5 mb-5">
				...
				<div>
					<div class=" is-size-14px">{{
						$t('Please invite more friends who are concerned about family and data privacy to join and use Falah OS.')
						}}
					</div>
...
export default {
	...
	data() {
		return {
			timer: 0,
			updateTimer: 0,
			githubUrl: `https://raw.githubusercontent.com/IceWhaleTech/logo/main/falahos/0.4/falahos_social_share.png`,
			shareTitle: `I'm using Falah OS, a simple, easy-to-use, elegant open-source home cloud system, try it like me.`,
			shareSites: [
				'facebook',
				'twitter',
				'reddit'
	...
	methods: {
		...
		getUpdateLogs() {
			this.updateTimer = setInterval(() => {
				this.$api.file.getContent(`/var/log/falahos/upgrade.log`).then(res => {

					this.updateLogs = res.data.data;
					if (this.updateLogs.includes(`Falah OS upgrade successfully`)) {
						clearInterval(this.updateTimer);
						setTimeout(() => {
							location.reload();
						}, 1000);
					} else if (this.updateLogs.includes(`Falah OS upgrade failed`)) {
