...
export default {
  ...
  computed: {
    ...
    appIcon() {
      return this.configData['x-falahos'].icon
    },
    appendNetworks() {
  ...
  methods: {
     ...
     */
    changeIcon(image) {
      // set this.configData['x-falahos'].icon
      this.configData['x-falahos'].icon = this.getIconFromImage(image)
    },
    ...
    async parseComposeYaml(val) {
      try {
        ...
        // this.current_service = yaml["x-falahos"].main;
        this.current_service = Object.keys(yaml.services)[0]
        // 解析 services，并将其赋值到 configData.services中。

        // set top level x-falahos data
        this.configData['x-falahos'] = merge(this.configData['x-falahos'], yaml['x-falahos'])
    ...
    parseComposeItem(composeServicesItemInput) {
      ...
      */
        ...
        if (isString(item)) {
          ...
          if (ii.length > 1) {
            return {
              type: 'bind',
              target: ii[1],
              source: this.volumeAutoCheck(ii[1], ii[0], lowerFirst(this.configData['x-falahos'].title.en_us)),
            }
          }
          else {
            return {
              type: 'bind',
              target: ii[0],
              source: this.volumeAutoCheck(ii[0], '', lowerFirst(this.configData['x-falahos'].title.en_us)),
            }
          }
        }
        else if (item) {
      ...
      // privileged
      // relation issue: https://github.com/IceWhaleTech/Falah OS/issues/1264
        ...
        <ValidationObserver :ref="`${key}valida`">
          ...
          <ValidationProvider v-slot="{ errors, valid }" name="composeAppName" rules="required">
            <b-field :label="`${$t('App Name')} *`" :message="$t(errors)" :type="{ 'is-danger': errors[0], 'is-success': valid }">
              <b-input :placeholder="$t('e.g.,Your App Name')" :value="ice_i18n(configData['x-falahos'].title)" @blur="(E) => (configData['x-falahos'].title.custom = E.target._value)" />
            </b-field>
          </ValidationProvider>
          ...
          <b-field v-if="key === firstAppName" :label="$t('Icon URL')">
            ...
            <b-input v-model="configData['x-falahos'].icon" :placeholder="$t('Your custom icon URL')" expanded />
          </b-field>

          <b-field v-if="key === firstAppName" label="Web UI">
            <b-select v-model="configData['x-falahos'].scheme">
              <option value="http">
                http://
              </option>
              <option value="https">
            ...
            <b-input v-model="configData['x-falahos'].hostname" :placeholder="baseUrl" expanded />
            <b-autocomplete v-model="configData['x-falahos'].port_map" :data="bridgePorts(configData.services)" :open-on-focus="true" :placeholder="$t('Port')" class="has-colon" field="hostname" @select="(option) => (portSelected = option)" />
            <b-input v-model="configData['x-falahos'].index" :placeholder="`/index.html ${$t('[Optional]')}`" expanded />
          </b-field>
