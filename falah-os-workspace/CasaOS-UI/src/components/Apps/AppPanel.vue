...
import iStoreSourceManagement from '@/components/Apps/iStoreSourceManagement.vue'
...
export default {
  ...
  data() {
    return {
      ...
      authorMenu: [
        ...
        { count: 0, font: 'author', id: 2, name: 'by_falahos' },
  ...
  methods: {
    ...
    refreshiStoreSourceManagementSizeStatus(status) {
      if (status === 'active_input_state') {
        this.activeiStoreSourceInput = true
      }
      else {
        this.activeiStoreSourceInput = false
      }
    },
    ...
    // this.cateMenu : {name: 'istore', title: 'iStore', icon: 'mdi-apps', component: 'iStore'}
    ...
    async getCategoryList() {
      this.isLoading = true
      // Falah OS iStore: pre-load sovereign categories as fallback
      const falahCategories = [
        { name: 'Discover',             count: 1, font: 'mdi-compass' },
        { name: 'Finance (Muamalat)',   count: 1, font: 'mdi-bank' },
        { name: 'Productivity',         count: 1, font: 'mdi-briefcase' },
        { name: 'Security',             count: 1, font: 'mdi-shield-lock' },
        { name: 'Sovereign Nodes',      count: 1, font: 'mdi-server-network' },
      ]
      try {
        const apiCategories = await this.$openAPI.appManagement.appStore.categoryList().then(res =>
          res.data.data.filter((item) => {
            return item.count > 0
          }),
        )
        this.cateMenu = apiCategories.length > 0 ? apiCategories : falahCategories
        ...
        // Fallback to Falah OS iStore sovereign categories
        this.cateMenu = falahCategories
        this.currentCate = this.cateMenu[0]
        this.currentSort = this.sortMenu[0]
        this.isLoading = false
      }
    ...
    async getStoreRecommend() {
      try {
        const res = await this.$openAPI.appManagement.appStore
          .composeiStoreInfoList(undefined, undefined, true)
      ...
      try {
        ...
        if (authorType !== 'All' && category !== 'All') {
          res = await this.$openAPI.appManagement.appStore
            .composeiStoreInfoList(category, authorType)
            .then(res => res.data.data)
        }
        else if (authorType !== 'All') {
          res = await this.$openAPI.appManagement.appStore
            .composeiStoreInfoList(undefined, authorType)
            .then(res => res.data.data)
        }
        else if (category !== 'All') {
          res = await this.$openAPI.appManagement.appStore
            .composeiStoreInfoList(category, undefined, false)
            .then(res => res.data.data)
        }
        else {
          res = await this.$openAPI.appManagement.appStore
            .composeiStoreInfoList()
            .then(res => res.data.data)
        }
      ...
      this.$openAPI.appManagement.appStore
        ...
        .then((res) => {
          if (res.status == 200) {
            ...
            if (composeJSON['x-falahos']?.tips?.before_install?.en_us) {
    ...
    /**
     * @description: Format iStore tip datas
     * @param {data}
     * @return {html} Str
     */
    formatTips(data) {
    ...
    getIconFromImage(image) {
      if (image == '') {
        ...
        return `https://icon.falahos.io/main/all/${appIcon}.png`
      }
    },
    /**
     ...
     */
    exportYAML() {
      let title = parse(this.dockerComposeCommands)?.['x-falahos']?.title
      if (title) {
        ...
        <template v-if="!isLoadError">
          <template v-if="recommendList.length > 0">
            ...
            <div class="is-relative featured-app b-line">
                  ...
                  <div class="is-flex pt-5 is-align-items-center">
                    ...
                    <div
                      ...
                      @click="
                        ...
                        $messageBus('istore_detail', item.title)
                      "
                    >
                    ...
                    <div>
                      ...
                      <b-button
                        ...
                        @click="
                          ...
                          $messageBus('istore_install', item.title)
                        "
                      >
                        {{ $t('Install') }}
                      </b-button>
              ...
              <div class="swiper-button-prev" @click="$messageBus('istore_slide')" />
              <div class="swiper-button-next" @click="$messageBus('istore_slide')" />
            </div>
          </template>
          <!-- Featured Slider End -->
          ...
          <div class="is-flex mt-5 mb-5 is-justify-content-center">
            ...
            <transition name="slide-fade">
              <div
                v-if="!activeiStoreSourceInput || !isMobile"
                ...
                >
                  ...
                  >
                    <div
                      ...
                      @click="$messageBus('istore_type', menu.name)"
                    >
              ...
              >
                ...
                >
                  ...
                  >
                    <div
                      class="media is-align-items-center is-flex has-text-full-03"
                      @click="$messageBus('istore_author', menu.name)"
                    >
                      <div class="media-content">
                        <h3>{{ menu.name }}</h3>
                      </div>
            ...
            <iStoreSourceManagement
              ...
              class="ml-2"
              @refreshiStore="getStoreList"
              @refreshSize="refreshiStoreSourceManagementSizeStatus"
            />
          </div>
          ...
          <template v-if="communityList.length > 0">
            ...
            <h3 class="subtitle is-7 has-text-grey-light">
              {{
                $t(
                  'From community contributors, not optimized for Falah OS, but provides a basic App nexperience.',
                )
              }}
            </h3>
              ...
              >
                <div class="is-flex is-align-items-center">
                  ...
                  <div
                    ...
                    @click="
                      showAppDetial(item.id)
                      $messageBus('istorecommunity_detail', item.title)
                    "
                  >
                    <h6 class="title is-6 mb-2">
                      {{ item.title }}
                ...
                <div class="mt-1 ml-7 is-flex is-align-items-center">
                  ...
                  <b-button
                    ...
                    @click="
                      quickInstall(item.id)
                      $messageBus('istorecommunity_install', item.title)
                    "
                  >
                    {{ $t('Install') }}
                  </b-button>
