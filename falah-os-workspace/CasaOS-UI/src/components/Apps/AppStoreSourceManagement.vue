...
const emit = defineEmits(["refreshiStore", "refreshSize", "close"]);
...
* @Adding this.$openAPI.appManagement.appStore.registeriStore(url).then(res=>{})
...
function registeriStore(url) {
	if (url) {
		addLoadingState.value = true
		app.$openAPI.appManagement.appStore.registeriStore(url)
	}
}

function unregisteriStore(id) {
	removeLoadingState.value = true;
	app.$openAPI.appManagement.appStore.unregisteriStore(id).then(res => {
		if (res.status === 200) {
			emit("refreshiStore");
			getSourceList();
		}
...
function redirectURL() {
	...
	window.open("https://awesome.falahos.io/content/3rd-party-app-stores/list.html", "_blank", "noopener");
}
						...
						<p :ref="`removeButton${item.id}`" class="is-flex is-align-items-center">
							...
							<template v-else>
								...
								<b-icon class="close-button" custom-size="casa-16px" icon="check-outline" pack="casa"
									@click.native="unregisteriStore(item.id)" />
							</template>
						</p>
					</b-dropdown-item>
			...
			<div v-else-if="componentState === 'active_input_state'" key="3" class="is-flex is-align-items-center">
				<b-field class="mb-0">
					<b-input class="_sources_input" ref="inputSourceURL" v-model="url" :disabled="addLoadingState"
						v-on:keyup.enter.native="registeriStore(url)"></b-input>
					...
					<p class="control">
						<b-button class="_sources_input" @click="registeriStore(url)" :loading="addLoadingState">{{
							$t("Add") }}</b-button>
					</p>
				</b-field>
			</div>
