...
export default {
	...
	methods: {
		async parseFeed() {			
			let params = await this.$api.file.getContent('/var/lib/falahos/baseinfo.conf').then(res => {
			...
			let feed = await parse('https://blog-falahos.zimaspace.com/feed/tag/dashboard/?key=' + stringify)
