...
func (a *ComposeApp) StoreInfo(includeApps bool) (*codegen.ComposeAppStoreInfo, error) {
	ex, ok := a.Extensions[common.ComposeExtensionNameXFalahOS]
	if !ok {
		return nil, ErrComposeExtensionNameXFalahOSNotFound
	}
	...
	isUncontrolled, ok := a.Extensions[common.ComposeExtensionNameXFalahOS].(map[string]interface{})[common.ComposeExtensionPropertyNameIsUncontrolled].(bool)
	if ok {
	...
	if includeApps {
		...
		for _, app := range a.Apps() {
			...
			if err != nil {
				if err == ErrComposeExtensionNameXFalahOSNotFound {
					logger.Info("App does not have x-falahos extension - skipping", zap.String("app", app.Name))
					continue
				}
...
func (a *ComposeApp) SetStoreAppID(storeAppID string) (string, bool) {
	...
	extension, ok := a.Extensions[common.ComposeExtensionNameXFalahOS]
	if !ok {
		logger.Info("compose app does not have x-falahos extension - might not be a compose app for Falah OS", zap.String("app", a.Name))
		return "", false
	}

	composeAppStoreInfo, ok := extension.(map[string]interface{})
	if !ok {
		logger.Info("compose app does not have valid x-falahos extension - might not be a compose app for Falah OS", zap.String("app", a.Name))
		return "", false
	}
...
func (a *ComposeApp) SetTitle(title, lang string) {
	...
	if !ok {
		...
		a.Extensions[common.ComposeExtensionNameXFalahOS] = extension
	}
	...
	if !ok {
		logger.Info("compose app does not have valid title map in its x-falahos extension - might not be a compose app for Falah OS", zap.String("app", a.Name))
		return
	}
...
func NewComposeAppFromYAML(yaml []byte, skipInterpolation, skipValidation bool) (*ComposeApp, error) {
	tmpWorkingDir, err := os.MkdirTemp("", "falahos-compose-app-*")
	if err != nil {
		return nil, err
	}
...
func (a *ComposeApp) SetUncontrolled(uncontrolled bool) error {
	xCasaos := a.Extensions[common.ComposeExtensionNameXFalahOS]
	...
	if !ok {
		...
		return ErrComposeExtensionNameXFalahOSNotFound
	} else {
		xCasaosMap[common.ComposeExtensionPropertyNameIsUncontrolled] = uncontrolled
		a.Extensions[common.ComposeExtensionNameXFalahOS] = xCasaosMap
	}
