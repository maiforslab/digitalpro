...
import (
	...
	"github.com/IceWhaleTech/Falah OS/pkg/cache"
	...
	"github.com/IceWhaleTech/Falah OS/route"
...
var (
	...
	//go:embed api/falahos/openapi.yaml
	_docYAML string

	//go:embed build/sysroot/etc/falahos/falahos.conf.sample
	_confSample string
...
func main() {
	...
	urlFilePath := filepath.Join(config.CommonInfo.RuntimePath, "falahos.url")
	...
	if supported, err := daemon.SdNotify(false, daemon.SdNotifyReady); err != nil {
		logger.Error("Failed to notify systemd that falahos main service is ready", zap.Any("error", err))
	} else if supported {
		logger.Info("Notified systemd that falahos main service is ready")
	} else {
	...
	logger.Info("Falah OS main service is listening...", zap.Any("address", listener.Addr().String()))
