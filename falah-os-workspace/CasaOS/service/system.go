...
type SystemService interface {
	...
	GetFalahOSLogs(lineNumber int) string
	UpdateAssist()
...
func (c *systemService) GenreateSystemEntry() {
	modelsPath := "/var/lib/falahos/www/modules"
...
func (s *systemService) UpdateSystemVersion(version string) {
	...
	// go command2.OnlyExec("curl -fsSL https://raw.githubusercontent.com/LinkLeong/falahos-alpha/main/update.sh | bash")
	if len(config.ServerInfo.UpdateUrl) > 0 {
		go command.OnlyExec("curl -fsSL " + config.ServerInfo.UpdateUrl + " | bash")
	} else {
		osRelease, _ := file.ReadOSRelease()
		go command.OnlyExec("curl -fsSL https://get.falahos.io/update?t=" + osRelease["MANUFACTURER"] + " | bash")
	}
...
func (s *systemService) GetFalahOSLogs(lineNumber int) string {
