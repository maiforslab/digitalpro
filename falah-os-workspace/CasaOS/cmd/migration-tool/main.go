 ...
 * @FilePath: /Falah OS/cmd/migration-tool/main.go
 * @Description:
...
const (
	falahosServiceName = "falahos.service"
)
...
func init() {
	...
	if !*forceFlag {
		serviceEnabled, err := systemctl.IsServiceEnabled(falahosServiceName)
		if err != nil {
			panic(err)
		}

		if serviceEnabled {
			_logger.Info("%s is already enabled. If migration is still needed, try with -f.", falahosServiceName)
			os.Exit(1)
		}
	}
